...
export function usePanZoom(
  ...
  const zoomIn = useCallback(() => {
    const newZ = Math.min(zoom * 1.2, 5)
    const svg = svgRef.current!
    const cx = svg.clientWidth / 2
    const cy = svg.clientHeight / 2
    animateTo(
      newZ,
      cx - (cx - pan.x) * (newZ / zoom),
      cy - (cy - pan.y) * (newZ / zoom),
    )
  }, [svgRef, zoom, pan, animateTo])

  const zoomOut = useCallback(() => {
    const newZ = Math.max(zoom * 0.8, 0.1)
    const svg = svgRef.current!
    const cx = svg.clientWidth / 2
    const cy = svg.clientHeight / 2
    animateTo(
  ...
  const panToNode = useCallback((nodeId: string) => {
    ...
    animateTo(zoom, cw / 2 - node.x * zoom, ch / 2 - node.y * zoom)
  }, [svgRef, nodes, zoom, pan, animateTo])
