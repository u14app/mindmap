...
const TAG_COLORS = ['#3B82F6', '#8B5CF6', '#EC4899', '#F59E0B', '#10B981', '#6366F1']

export const tagsPlugin: MindMapPlugin = {
  name: 'tags',

  transformNodeData(node, _rawText) {
    const match = node.text.match(TAG_TRAILING_RE)
    if (!match) return node

    const tagsPart = match[1]
    const cleanText = node.text.slice(0, node.text.length - tagsPart.length).trim()
    const tags: string[] = []
    const tagRegex = /#([\w-]+)/g
    let m
  ...
  exportNodeDecoration(node, theme) {
    ...
    const multiLineOffset = node.multiLineContent ? node.multiLineContent.length * lineHeight : 0
    ...
    for (let i = 0; i < node.tags.length; i++) {
      ...
      parts.push(`<rect x="${tagX}" y="${tagY}" width="${tagWidth}" height="${tagHeight}" rx="3" fill="${color}" opacity="0.15"/>`)
      parts.push(`<text x="${tagX + tagWidth / 2}" y="${tagY + tagHeight / 2}" text-anchor="middle" dominant-baseline="central" font-size="${tagFontSize}" fill="${color}" font-family="${fontFamily}">${escapeXml(tag)}</text>`)
